...
with IML as (
    with t_total AS (
            with t_cost AS (
                with t_routes AS (
                    with t_orders AS (
                        SELECT fd.delivery_gk delivery_id, fd.journey_gk journey_id, ds.delivery_status_desc status,
                         date(fd.scheduled_at) AS date_key, fd.pickup_location_key, courier_gk
                        ...
                        WHERE fd.country_symbol = 'RU'
                         and company_gk = 200024062

                         and ds.delivery_status_desc IN ( 'completed' , 'not_delivered')
                         and date(fd.scheduled_at) >= date'2020-05-01'
                        --ORDER BY delivery_id desc
                        ) -- orders

                    (SELECT
                        t1.journey_id, t1.pickup_location_key, t1.date_key,
                        count (t1.delivery_id) AS deliveries,
                        sum(CASE when t1.status = 'completed' and courier_gk <> 200013 THEN 1 ELSE 0 end ) AS completed,
                        sum(CASE when t1.status = 'not_delivered' THEN 1 ELSE 0 end ) AS not_delivered,
                        sum(CASE when t1.status = 'completed' and courier_gk <> 200013 THEN 1 ELSE 0 end )*1.0/count (t1.delivery_id)*1.0 AS CR
            ...
            (SELECT
                   t2.journey_id, t2.pickup_location_key, t2.date_key,
             ...
             (SELECT
              t3.journey_id, t3.pickup_location_key,
              t3.date_key,
              t3.deliveries,
              t3.completed,
              t3.CR,
    ...
    FROM t_total AS t
    LEFT JOIN emilia_gettdwh.dwh_dim_locations_v AS loc ON t.pickup_location_key = loc.location_key
    GROUP BY 1,2,3)
...
(CASE WHEN lob_desc = 'Deliveries - B2B'
    ...
    ELSE (CASE
                --iml correction
        WHEN p2p.customer_total_cost_inc_vat - driver_total_cost_inc_vat >0
                --iml correction
        THEN round((p2p.customer_total_cost_inc_vat
    - driver_total_cost_inc_vat)/1.2,2)
                --iml correction
        ELSE  p2p.customer_total_cost_inc_vat - driver_total_cost_inc_vat END
        )
    END) AS buy_sell

from p2p
left join IML on p2p.company_gk = iml.company_gk and iml.date_key = p2p.date_key
and p2p.city = iml.city)
)
(
...
(select  platform ,  client_type, subperiod,
        ...
        sum(case when order_status = 'Completed' then gross_orders end) completed_orders, --name_internal,
        sum(buy_sell)+sum(driver_total_commission_exc_vat) TR,
        sum(customer_total_cost) GMV,
        sum(buy_sell) buy_sell,
        sum(customer_total_cost_inc_vat) customer_total_cost_inc_vat,
        sum(driver_total_cost_inc_vat) driver_total_cost_inc_vat,
        (sum(buy_sell)+sum(driver_total_commission_exc_vat))*1.00 / sum(customer_total_cost) TR_perc

from main
where subperiod in ('W37', 'W38','W39','W36')
and client_type = '3. C2C'
group by 1,2,3)
*/
...
, p2p as (
    ...
    WHERE fo.country_key=2
      ...
      and (fo.order_status_key = 7 or (fo.order_status_key=4 and fo.driver_total_cost>0))
      AND fo.date_key >= date'2020-05-01'
      and fo.ordering_corporate_account_gk not in ();
  )
-- day level + IML
(SELECT p2p_fo.date_key, p2p_fo.platform,
       --p2p_fo.hour_key,
    ...
    with t_total AS (
            with t_cost AS (
                with t_routes AS (
                    with t_orders AS (
                        SELECT fd.delivery_gk delivery_id, fd.journey_gk journey_id, ds.delivery_status_desc status,
                         date(fd.scheduled_at) AS date_key, fd.pickup_location_key
...
(select  platform ,  client_type, subperiod,
        ...
        sum(customer_total_cost) GMV,
        sum(buy_sell) buy_sell,
        sum(driver_total_commission_exc_vat) driver_total_comission_exc_vat,
        (sum(buy_sell)+sum(driver_total_commission_exc_vat))*1.00 / sum(customer_total_cost) TR_perc
...
-- TODO figure out why C2C wrong Buy Cell (driver total comission is correct)
-- driver total cost inc vat is the the same
-- GMV (customer total cost) is the same
-- customer_total_cost_inc_vat is the same




-- buy sell [customer total cost - driver total cost]