drop table if exists temp.delivery_leads
create table temp.delivery_leads
as;
    ...
    distinct
            ...
            when d.fleet_gk in (200014202,200016265,200016266,200016267,200016359,200016361,200023819) then 'Gorizont'
            when d.fleet_gk in (200017083,200017177,200017412,200017342,200017205,200017203, 200017524,200017523,200017517,200017430,200023811) then 'Scouts'
            when d.fleet_gk in (200037169, 200037181) then 'MO'
            ...
            nullif(d.ftp_date_key, date'1900-1-1') first_ftr,
                        -- while fixing
            --max(case when ride_type = 'ReFTRD' then try(date(rftr.date_key)) end) reFTR,
            max(case when ride_type = 'reFTR' then try(date(rftr.date_key)) end) reFTR,
            max(case when d.driver_gk = prog.driver_gk then try(date(lead_date)) end) as "external_source_lead_date"
        ...
        left join analyst.reftr_delivery rftr on d.driver_gk = rftr.driver_gk
            ...
            (
                select
                d.driver_gk,
                ( case when try(date(rftr.date_key)) is null then d.ftp_date_key else try(date(rftr.date_key)) end)
                 between try(cast("start" as date)) and try(cast("end" as date))
                ...
                and (
                    (( case when try(date(rftr.date_key)) is null then d.ftp_date_key else try(date(rftr.date_key)) end)
                                            between try(cast("start" as date)) and try(cast("end" as date)))
                    or (d.registration_date_key between try(cast("start" as date)) and try(cast("end" as date)))
                        )
            ...
            (
                    ...
                    max(try(date(lead_date))) as lead_date
...
grant all privileges on temp.delivery_leads to role public with grant option

select max(lead_date)
from (
         select distinct d.phone                      phone_number,
                         "name",
                         d.driver_name                registration_name,
                         d.fleet_gk,
                         driver_gk,
         ...
         where "source" <> 'source'                                                         --filter the bug that occured because of union of tables in google sheet
           ...
           and phone_number not in ('8', '', '9999999999', ' ', '3333333333', '2222222222') -- dummy phones
           and phone_number is not null
           and date(lead_date) >= date '2021-03-20'
